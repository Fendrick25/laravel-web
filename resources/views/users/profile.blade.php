...
@section('content')
    ...
    <div class="d-flex justify-content-around text-white">
        <div class="row mt-5 d-flex justify-content-center">
            ...
            @if (Auth::user()->image_source !== null)
                <div class="w-50">
                    <button type="button" data-bs-toggle="modal" data-bs-target="#uploadImage"
                        ...
                        <img src="{{Auth::user()->imageUrl}}" alt="" style="width: 100%">
                    </button>
                </div>
            @else
                <div class="w-50">
            ...
            @endif
            @auth
                <p class="row-3 text-center">{{Auth::user()->name}}</p>
                <p class="row-3 text-center">{{Auth::user()->email}}</p>
            @endauth
        </div>
            ...
            <form action="/profile/update/{{ Auth::id() }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row m-3 p-2">
                    ...
                    <input type="text" name="name" id=""
                           class="col-5 p-2 bg-dark border border-white text-white" value="{{Auth::user()->name}}">
                </div>
            ...
            <div class="modal-content bg-dark">
                <form action="/profile/image/update/{{ Auth::id() }}" method="POST" class="text-white"
                    ...
                    <div class="modal-body text-white">
                        <input class="form-control bg-dark text-white" type="text" name="imageUrl" id=""
                               placeholder="Image URL" value="{{Auth::user()->imageUrl}}"> <br>
